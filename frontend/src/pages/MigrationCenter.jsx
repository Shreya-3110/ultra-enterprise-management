        ...
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                   ...
                   <p className="text-slate-500 dark:text-slate-400 font-medium mt-2">Enterprise-grade data migration with Sandbox simulation.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-1 bg-white dark:bg-[#111827] p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl flex flex-col justify-between h-full">
                    <div>
                        <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mb-6">
                            <Upload size={24} />
                        </div>
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">1. Upload Source</h3>
                        ...
                        <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-slate-200 dark:border-slate-700/50 rounded-[2rem] cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/80 dark:bg-slate-800/50 transition-colors group">
                    ...
                    {fileData.length > 0 && (
                        <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
                             <div className="flex items-center justify-between mb-6">
                                <span className="text-xs font-black text-slate-900 dark:text-white uppercase">Input Rows</span>
                                <span className="text-xl font-black text-blue-600">{fileData.length}</span>
                             </div>
                             <button 
                                onClick={runSimulation}
                                disabled={loading || status === 'committed'}
                                className="w-full py-4 bg-slate-900 dark:bg-slate-950 text-white rounded-2xl font-bold text-sm uppercase tracking-widest hover:bg-black transition-all flex items-center justify-center gap-3"
                             >
                ...
                <div className="md:col-span-2 bg-slate-900 dark:bg-slate-950 p-10 rounded-[3rem] shadow-2xl shadow-slate-900/40 relative overflow-hidden">
                    ...
                    <div className="relative h-full flex flex-col">
                        <div className="flex items-center justify-between mb-10">
                            ...
                            <div className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${status === 'simulated' ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-white dark:bg-[#111827]/5 border-white/10 text-white/40'}`}>
                        ...
                        {!report ? (
                            <div className="flex-1 flex flex-col items-center justify-center text-center py-20">
                                <div className="w-16 h-16 bg-white dark:bg-[#111827]/5 rounded-full flex items-center justify-center mb-6">
                                    <Database className="text-slate-700 dark:text-slate-300" size={32} />
                                </div>
                                <p className="text-slate-500 dark:text-slate-400 max-w-xs font-medium">Please upload a source file and start simulation to see the sandbox results.</p>
                            </div>
                        ) : (
                            <div className="space-y-10 flex-1">
                                ...
                                <div className="bg-white dark:bg-[#111827]/5 border border-white/10 rounded-[2rem] flex-1 min-h-[300px] overflow-hidden flex flex-col">
                                    <div className="p-5 border-b border-white/5 bg-white dark:bg-[#111827]/2">
                                        <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Error Log & Conflict Audit</p>
                                    </div>
                                    <div className="flex-1 overflow-y-auto p-6 space-y-4 max-h-[350px]">
                                        ...
                                        {report.errors.map((err, i) => (
                                            <div key={i} className="flex items-start gap-4 p-4 bg-white dark:bg-[#111827]/2 border border-white/5 rounded-2xl group hover:bg-white dark:bg-[#111827]/5 transition-colors">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-2">
                                                       ...
                                                       <span className="text-[9px] text-slate-600 dark:text-slate-400 font-bold italic">Validation check failed</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
