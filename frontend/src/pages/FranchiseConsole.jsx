...
const FranchiseConsole = () => {
            ...
            <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-500 pb-20">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-slate-500 font-medium mt-2">Consolidated revenue split monitoring across all franchises.</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden">
                    <div className="p-8 border-b border-slate-50 bg-slate-50/30">
                        <h3 className="font-black text-slate-900 uppercase tracking-tight">Consolidated Franchise Audit</h3>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {data?.details.map((b) => (
                                    <tr key={b.branchId} className="hover:bg-slate-50/50 transition-all">
                                        <td className="px-8 py-6">
                                            <p className="font-bold text-slate-900 uppercase tracking-tight">{b.branchName}</p>
                                        </td>
                                        <td className="px-8 py-6 text-right font-bold text-slate-600">₹{b.grossCollections.toLocaleString()}</td>
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-500 pb-20">
            <div>
                <h1 className="text-4xl font-black text-slate-900 tracking-tighter">Franchise Earnings</h1>
                <p className="text-slate-500 font-medium mt-2">Transparent revenue breakdown and organizational splits.</p>
            </div>
            ...
            <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl text-white relative overflow-hidden">
                ...
                <div className="relative space-y-10">
                    <div className="grid grid-cols-2 gap-10 border-b border-white/5 pb-10">
                        <div>
                            <p className="text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Total Collections</p>
                            <p className="text-4xl font-black">₹{ownerData?.gross.toLocaleString()}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Head Office Share ({ownerData?.royaltyPct}%)</p>
            ...
            <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-xl flex items-center gap-6">
                <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center">
                    <ShieldCheck size={32} />
                </div>
                <div>
                    <h4 className="font-bold text-slate-900">Agreement Compliance Verified</h4>
                    <p className="text-sm text-slate-500">Your revenue split is calculated according to the FOFO model agreement.</p>
                </div>
            </div>
        </div>
    );
...
const RevenueCard = ({ title, value, icon, color }) => (
    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl flex items-center gap-6">
        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg ${color === 'blue' ? 'bg-blue-600 text-white shadow-blue-500/30' : 'bg-slate-900 text-white shadow-slate-900/30'}`}>
