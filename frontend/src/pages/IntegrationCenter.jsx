    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 sm:space-y-10 animate-in fade-in duration-500 pb-10 sm:pb-20 px-2 sm:px-0">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
                <div>
                   <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tighter flex items-center gap-3">
                      <Terminal className="text-blue-600" size={32} />
                      Integrations
                   </h1>
                   <p className="text-slate-500 dark:text-slate-400 text-sm sm:text-base font-medium mt-1 sm:mt-2">Connect Ultra Enterprise to your ecosystem via Webhooks & REST.</p>
                </div>
            </div>

            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
                {/* API Key Management */}
                <div className="xl:col-span-1 bg-white dark:bg-[#111827] p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl space-y-6 sm:space-y-8 flex flex-col">
                    <div>
                        <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mb-6">
                            <Key size={24} />
                        </div>
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Generate API Secret</h3>
                    ...
                    <form onSubmit={generateKey} className="space-y-4">
                        ...
                        <button 
                            disabled={generating}
                            className="w-full py-4 bg-slate-900 dark:bg-slate-950 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-black transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2"
                        >
                    ...
                    {newKey && (
                        <div className="p-6 bg-emerald-50 border border-emerald-100 rounded-2xl animate-in zoom-in duration-300">
                             ...
                             <div className="flex items-center gap-2 bg-white dark:bg-[#111827] p-3 border border-emerald-200 rounded-xl">
                ...
                <div className="xl:col-span-2 space-y-6 sm:space-y-8">
                    <div className="bg-slate-900 dark:bg-slate-950 rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-10 shadow-2xl text-white relative overflow-hidden">
                    ...
                    <div className="bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-10 shadow-xl space-y-6 sm:space-y-8">
                        <div className="flex items-center gap-3">
                            <Code className="text-blue-600" size={24} />
                            <h3 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Developer Documentation</h3>
                        </div>
                        
                        <div className="space-y-6">
                             <div>
                                <p className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest mb-3">1. Authenticate Request</p>
                                <div className="bg-slate-900 dark:bg-slate-950 rounded-2xl p-4 sm:p-6 font-mono text-[10px] sm:text-xs text-blue-300 leading-relaxed overflow-x-auto whitespace-nowrap sm:whitespace-normal">
                             ...
                             <div>
                                ...
                                <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                    <code className="text-[10px] sm:text-xs font-black text-slate-600 dark:text-slate-400 uppercase break-all">/api/v1/integration/webhooks/stripe</code>
                        ...
                        <div className="pt-6 border-t border-slate-50 dark:border-slate-800/50 flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400 font-medium italic">
...
const StatusCard = ({ label, status, lastActive }) => (
    <div className="bg-white dark:bg-[#111827]/5 border border-white/10 p-6 rounded-2xl flex justify-between items-start">
        <div>
            <p className="text-xs font-black text-white uppercase tracking-widest">{label}</p>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mt-1">Last Active: {lastActive}</p>
        </div>
