    ...
    <div className="space-y-8 max-w-6xl mx-auto pb-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/" className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800 rounded-full transition-colors">
            <ArrowLeft size={20} className="text-slate-600 dark:text-slate-400" />
          </Link>
          <div>
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">System Audit Logs</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">A transparent trail of all administrative actions</p>
          </div>
        </div>
          <button 
            onClick={() => generateAuditReport(logs)}
            className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/80 dark:bg-slate-800/50 transition-colors"
          >
            <Download size={14} />
            Export PDF
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm"
            value={searchTerm}
        ...
        <div className="relative">
          ...
          <select className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 rounded-xl focus:outline-none text-sm appearance-none cursor-pointer">
      ...
      {/* Logs Table */}
      <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 rounded-2xl overflow-hidden shadow-sm shadow-slate-100">
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                ...
                filteredLogs.map((log) => (
                  <tr key={log._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/80 dark:bg-slate-800/50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs">
                      <span className="flex items-center gap-1.5 font-bold tracking-tight px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700/50">
                        {getActionIcon(log.action)}
                        {log.action.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-medium text-slate-600 dark:text-slate-400 leading-relaxed">
                      {log.details}
                    </td>
                  </tr>
                ))
